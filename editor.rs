...
pub struct Editor {
    ...
    pub previous_current_char: usize,
    pub syntax_color_buffer: Vec<Color>,
}

pub fn editor_process(editor: &mut Editor, event: Event) -> ProcessResult {
    match event {
        //Esc
...
pub fn editor_syntax_highlighting(editor: &mut Editor) {
...
pub fn editor_draw(editor: &mut Editor) {
    let mut characters_count = 0;
    for line in editor.lines.iter() {
        characters_count += line.len();
    }
